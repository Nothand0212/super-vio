...
public:
  ...
  virtual int
  initOrtEnv( const utilities::Configuration& config )
  {
    return EXIT_SUCCESS;
  }
  ...
  inferenceImage( const utilities::Configuration& config, const cv::Mat& image_src, const cv::Mat& image_dst )
  {
  ...
  void extractNodesInfo( const IO& io, std::vector<char*>& node_names, std::vector<std::vector<int64_t>>& node_shapes, const std::unique_ptr<Ort::Session>& session_uptr, const Ort::AllocatorWithDefaultOptions& allocator )
  {
    ...
    // INFO( super_vio::logger, "num_nodes: {0}", num_nodes );
    ...
    auto processNode = [ & ]( size_t i ) {
      ...
      // INFO( super_vio::logger, "extractor node name: {0}", node_name_temp );
