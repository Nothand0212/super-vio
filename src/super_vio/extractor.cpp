...
int Extracotr::initOrtEnv( const utilities::Configuration& config )
{
  ...
  try
  {
    ...
    {
      INFO( super_vio::logger, "Using CUDA for extractor" );
    ...
    INFO( super_vio::logger, "Loading extractor model from {0}", config.extractor_path );

    m_uptr_session = std::make_unique<Ort::Session>( m_env, config.extractor_path.c_str(), m_session_options );

    // INFO( super_vio::logger, "Extractor model loaded" );
    ...
    INFO( super_vio::logger, "Extractor ort env initialized" );
  }
  catch ( const std::exception& e )
  {
    INFO( super_vio::logger, "Failed to initialize extractor ort env: {0}", e.what() );
    return EXIT_FAILURE;
  }
...
cv::Mat Extracotr::prePorcess( const utilities::Configuration& config, const cv::Mat& image, float& scale )
{
  ...
  // INFO( super_vio::logger, "image_in size: [{0}, {1}]", image_temp.cols, image_temp.rows );

  std::string fn{ "max" };
  std::string interp{ "area" };
  cv::Mat     image_result = utilities::normalizeImage( utilities::resizeImage( image_temp, config.image_size, scale, fn, interp ) );
  // INFO( super_vio::logger, "image_out size: [{0}, {1}], scale from 1.0 to {3}", image_result.cols, image_result.rows, scale_temp, scale );
...
int Extracotr::inference( const utilities::Configuration& config, const cv::Mat& image )
{
  ...
  try
  {
    ...
    INFO( super_vio::logger, "inference extractor time consumed: {0}", m_timer.tocGetDuration() );
    ...
    for ( const auto& tensor : output_tensors )
    {
      if ( !tensor.IsTensor() )
      {
        ERROR( super_vio::logger, "Output {0} is not a tensor", count );
      }

      if ( !tensor.HasValue() )
      {
        ERROR( super_vio::logger, "Output {0} is empty", count );
      }
  ...
  {
    ERROR( super_vio::logger, "Failed to inference image: {0}", e.what() );
    return EXIT_FAILURE;
  }
...
Features Extracotr::postProcess( const utilities::Configuration& config, std::vector<Ort::Value> tensor )
{
  ...
  try
  {
    ...
    // INFO( super_vio::logger, "key points shape: [{0}, {1}, {2}], size: {3}", points_shape[ 0 ], points_shape[ 1 ], points_shape[ 2 ], points_shape.size() );
    ...
    // INFO( super_vio::logger, "score shape: [{0}, {1}], size: {2}", score_shape[ 0 ], score_shape[ 1 ], score_shape.size() );
    ...
    // INFO( super_vio::logger, "descriptor shape: [{0}, {1}, {2}], size: {3}", descriptor_shape[ 0 ], descriptor_shape[ 1 ], descriptor_shape[ 2 ], descriptor_shape.size() );
...
std::pair<Features, std::vector<Region>> Extracotr::distributeKeyPointsDebug( const Features& key_points, const cv::Mat& image )
{
  ...
  //   {
  //     WARN( super_vio::logger, "Point number is {0}, m_point_num_is {1}, no need to distribute", key_points.getScores().size(), m_point_num );
  ...
  {
    INFO( super_vio::logger, "Regions number: {0} / {1}", queue_region.size(), last_region_size );
    ...
    // INFO( super_vio::logger, "temp_indexs size: {0}", temp_indexs.size() );

    cv::Rect2i temp_rect = temp_region.rectangle;
    // INFO( super_vio::logger, "temp_rect: [{0}, {1}, {2}, {3}]", temp_rect.x, temp_rect.y, temp_rect.width, temp_rect.height );
    ...
    {
      ERROR( super_vio::logger, "top_left_indexs is empty" );
    }
    ...
    {
      ERROR( super_vio::logger, "top_right_indexs is empty" );
    }
    ...
    {
      ERROR( super_vio::logger, "bottom_left_indexs is empty" );
    }
    ...
    {
      ERROR( super_vio::logger, "bottom_right_indexs is empty" );
    }
  ...
  while ( !queue_region.empty() )
  {
    ...
    {
      ERROR( super_vio::logger, "best_index is -1" );
    }
  }
...
Features Extracotr::distributeKeyPoints( const Features& key_points, const cv::Mat& image )
{
  ...
  while ( queue_region.size() < m_point_num && !queue_region.empty() )
  {
    INFO( super_vio::logger, "Regions number: {0}", queue_region.size() );
    Region temp_region = queue_region.front();
    queue_region.pop_front();
...
Features Extracotr::inferenceImage( const utilities::Configuration& config, const cv::Mat& image )
{
  ...
  {
#ifdef DEBUG
    // INFO( super_vio::logger, "inference image time consumed: {0}", timer.tocGetDuration() );
#endif
    return getKeyPoints();
  }
  else
  {
    ERROR( super_vio::logger, "Failed to inference image" );
    return Features{};
  }
}
